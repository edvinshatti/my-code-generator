...
N'
	...
	+ N')
	{
		' + @ClassName + N'.Result result = new ' + @TypeName + N'().Save(' + @Parameters + N');
			
		return new Result()
		{
			Type = result.Type == Payam.Library.ResultType.Success ? ResultType.Success : ResultType.Failed,
			Code = result.Code,
...
Select	@Method	=
N'
	public List<' + @TypeName + N'> ' + @TypeName + N'_SelectAll()
	{
		return new ' + @TypeName + N'().SelectAll();
	}
	'
...
Select	@Method	=
N'
	public ' + @TypeName + N' ' + @TypeName + N'_SelectBy' + @PrimaryKey + N'(' + @PrimaryKeyDataType + N' ' + @PrimaryKeyProp + N')
	{
		return new ' + @TypeName + N'().SelectBy' + @PrimaryKey + N'(' + @PrimaryKeyProp + N');
	}
	'
...
Select	@Method	=
N'
	public List<' + @TypeName + N'> ' + @TypeName + N'_Search('
	+ STUFF(
	...
	+ N')
	{
		return new ' + @TypeName + N'().Search(' + @Parameters + N');
	}
	'
...
N'
	...
	{
		' + @ClassName + N'.Result result = new ' + @TypeName + N'().DeleteBy' + @PrimaryKey + N'(' + @PrimaryKeyProp + N');
